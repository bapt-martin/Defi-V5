...
public class Plane {
    ...
    public int trisClippingPlane (Triangle triIn, Triangle triOut1, Triangle triOut2) {
        Vertex3D planePoint = this.getOrigin();
        Vector3D planeNorm = this.getNormal();

        planeNorm.normalized();
        ...
        for (int i = 0; i < 3; i++) {
            double dDistPoint = this.distPointToPlane(triIn.getVertices()[i]);
            if (dDistPoint >= 0) {
        ...
        if (nbPointsInside == 1) {
            ...
            vertsOut1[1] = this.vertexIntersectPlane(ptsInside[0],ptsOutside[0]);
            vertsOut1[2] = this.vertexIntersectPlane(ptsInside[0],ptsOutside[1]);
        ...
        if (nbPointsInside == 2) {
            ...
            vertsOut1[2] = this.vertexIntersectPlane(ptsInside[0],ptsOutside[0]);

            vertsOut2[0] = ptsInside[1];
            vertsOut2[1] = this.vertexIntersectPlane(ptsInside[0],ptsOutside[0]);; // dans mon livre faut inverser les 2
            vertsOut2[2] = this.vertexIntersectPlane(ptsInside[1],ptsOutside[0]);
...
//            triOut2.setColor(Color.BLUE);
    ...
    public Vertex3D vertexIntersectPlane(Vertex3D pLineStart, Vertex3D pLineEnd) {
        ...
        Vector3D vectLineIntersecting = vectLineStartToEnd.scale(dLengthIntersecting);

        return pLineStart.translate(vectLineIntersecting);
    }
